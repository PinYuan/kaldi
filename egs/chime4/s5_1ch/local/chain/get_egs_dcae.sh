...
if [ $# != 6 ]; then
  echo "Usage: $0 [opts] <data> <chain-dir> <lattice-dir> <targets-scp> <clean-targets-scp> <egs-dir>"
  echo " e.g.: $0 data/train exp/tri4_nnet exp/tri3_lats data/train/feats.scp data/train_clean_orig/feats.scp exp/tri4_nnet/egs"
  echo ""
...
for f in $data/feats.scp $latdir/lat.1.gz $latdir/final.mdl \
         $chaindir/{0.trans_mdl,tree,normalization.fst} $targets_scp $clean_targets_scp $extra_files; do
...
case $target_type in
  "dense") 
    ...
    clean_targets="ark:$clean_targets_scp | copy-feats scp:- ark:- |"
    clean_valid_targets="ark:$clean_targets_scp | copy-feats scp:- ark:- |"
    clean_train_subset_targets="ark:$clean_targets_scp | copy-feats scp:- ark:- |"
  ...
  (
    ...
    $cmd $dir/log/create_valid_subset.log \
      ...
      nnet3-chain-get-egs-dcae $ivector_opts --srand=$srand --num-targets=$num_targets --target-clean=$target_clean \
        ...
        "$valid_feats" ark,s,cs:- "$valid_targets" "$clean_valid_targets" "ark:$dir/valid_all.cegs" || exit 1
    $cmd $dir/log/create_train_subset.log \
      ...
      nnet3-chain-get-egs-dcae $ivector_opts --srand=$srand --num-targets=$num_targets --target-clean=$target_clean\
        $egs_opts --normalization-fst-scale=$normalization_fst_scale \
        $trans_mdl_opt $chaindir/normalization.fst \
        "$train_subset_feats" ark,s,cs:- "$train_subset_targets" "$clean_train_subset_targets" "ark:$dir/train_subset_all.cegs" || exit 1
    wait
...
if [ $stage -le 4 ]; then
  ...
  $cmd --max-jobs-run $max_jobs_run JOB=1:$nj $dir/log/get_egs.JOB.log \
    ...
    nnet3-chain-get-egs-dcae $ivector_opts --srand=\$[JOB+$srand] $egs_opts --num-targets=$num_targets --target-clean=$target_clean \
     ...
     "$feats" ark,s,cs:- "$targets" "$clean_targets" ark:- \| \
