...
affix=_dcae_l3  #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if $decode_only; then
  ...
  if [ $stage -le 2 ]; then
    # make hires mfcc for clean data
    clean_data=tr05_orig_clean
    utils/copy_data_dir.sh data/${clean_data} data/${clean_data}_hires

    # do volume-perturbation on the training data prior to extracting hires
    # features; this helps make trained nnets more invariant to test data volume.
    utils/data/perturb_data_dir_volume.sh data/${clean_data}_hires

    for datadir in ${clean_data}; do
      steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
        --cmd "$train_cmd" data/${datadir}_hires
      steps/compute_cmvn_stats.sh data/${datadir}_hires
      utils/fix_data_dir.sh data/${datadir}_hires
...
dir=exp/chain${nnet3_affix}/tdnn${affix}_e${num_of_epoch}_sp
if [ target_clean ];then
    dir=exp/chain${nnet3_affix}/tdnn${affix}_e${num_of_epoch}_clean_sp
fi
train_data_dir=data/${train_set}_sp_hires
clean_data_dir=data/tr05_orig_clean_hires
...
if [ $stage -le 15 ]; then
  ...
  # additional layer of DcAE
  relu-batchnorm-layer name=tdnn9 input=tdnn7 $opts dim=850
  relu-batchnorm-layer name=tdnn10 input=Append(0, tdnn8) $opts dim=850
  relu-batchnorm-layer name=tdnn11 $opts dim=850
  output-layer name=output_ae include-log-softmax=false learning-rate-factor=$weight max-change=1.0 objective-type=quadratic input=tdnn11 dim=40
...
if [ $stage -le 16 ]; then
  ...
  local/chain/train_dcae.py --stage=$train_stage \
    --cmd="$decode_cmd" \
