...
def prepare_train(kind):
    ...
    data_dir = f"data/train_{kind}_sp_hires"
    ...
    with open(f"data/{train_set}/wav.scp") as multi_file, \
        open(f"{timit_egs}/data/{train_set}/wav.scp") as clean_file, \
        open(f"data/train_{kind}_sp_hires/log/mix_wav.log", "w") as log_file:
    ...
    dump_files(train_set, data_dir, data)


def prepare_test(kind):
    clean_cmd_dict = dict()
    with open("data/test_eval92_5k_clean/wav.scp") as clean_file:
        for clean_line in tqdm.tqdm(clean_file):
            clean_key, clean_cmd = clean_line.strip().split(" ", 1)
            clean_cmd = clean_cmd[:-2]
            clean_cmd_dict[clean_key] = clean_cmd
            
    for data_set in ["dev_hires", "test_hires"]:
        ...
        with open(f"data/{data_set}/wav.scp") as multi_file,\
            ...
            for multi_line in tqdm.tqdm(multi_file):
                ...
                # compute the difference between 2 wav files and output a new wav
                cmd = f"sox -m -v 1 '|sox {multi_cmd} -c 1 -t wav -' -v -1 '|{clean_cmd}' {data_dir}/wav/{multi_key}.wav"
                p = subprocess.Popen(cmd, shell=True)
                p.communicate()
                data[multi_key] = f"{data_dir}/wav/{multi_key}.wav" # cmd
