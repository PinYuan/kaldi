...
if $use_ihm_ali; then
  ...
  dir=exp/$mic/chain${nnet3_affix}/train_from_scratch/CNN_TDNN_1C/FSFAE3/DEFAULT/SPECAUG/feam_mtlae_fbank-mfcc-t/${argu_desc}
...
if [ $stage -le 15 ]; then
  ...
  combine-feature-maps-layer name=combine_inputs input=Append(spec-augment, ivector-batchnorm, dae-batchnorm) num-filters1=1 num-filters2=5 num-filters3=1 height=40
...
if [ $stage -le 16 ]; then
  ...
  steps/chain/train_mtlae.py --stage 157 \
    --cmd "$decode_cmd" \
