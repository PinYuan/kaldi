...
argu_desc="e${num_of_epoch}_fdae${frame_weight_dae}_fdspae${frame_weight_dspae}_scale5e-1_il${initial_effective_lrate}_fl${final_effective_lrate}"
...
if $use_ihm_ali; then
  ...
  dir=exp/$mic/chain${nnet3_affix}/train_from_scratch/CNN_TDNN_1C/FSFAE3/DEFAULT/SPECAUG/feam_mtlae_fbank-mfcc-t_interpolation/${argu_desc}
...
if [ $stage -le 15 ]; then
  ...
  batchnorm-component name=dae-batchnorm input=prefinal-dae
  combine-feature-maps-layer name=combine_inputs input=Append(Scale(0.5, spec-augment), ivector-batchnorm, Scale(0.5, dae-batchnorm)) num-filters1=1 num-filters2=5 num-filters3=1 height=40
