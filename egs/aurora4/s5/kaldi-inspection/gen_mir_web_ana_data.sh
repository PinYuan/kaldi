...
lmwt=`cat $decode_dir/scoring/wer_details/lmwt`
wip=`cat $decode_dir/scoring/wer_details/wip`
steps/get_ctm_fast.sh \
    $frame_shift --print-silence true --lmwt $lmwt --wip $wip \
    $data $graph $decode_dir $decode_dir/ctm

python kaldi-inspection/word_ctm_to_mir_schema.py $data $decode_dir/ctm/ctm $decode_dir/mir
