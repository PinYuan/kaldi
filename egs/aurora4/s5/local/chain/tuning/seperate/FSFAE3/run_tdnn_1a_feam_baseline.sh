...
num_of_epoch=100
frame_weight=1
initial_effective_lrate=0.05
final_effective_lrate=0.05
...
dir=exp/chain${nnet3_affix}/seperate/FSFAE3/tdnn_${affix}_feam_baseline/${argu_desc}
...
if [ $stage -le 15 ]; then
  ...
  relu-renorm-layer name=tdnn_1 dim=512
  relu-renorm-layer name=tdnn_2 dim=512 input=Append(-1,0,1)
  relu-renorm-layer name=tdnn_3 dim=512 input=Append(-1,0,1)
  relu-renorm-layer name=tdnn_4 dim=512 input=Append(-3,0,3)
  relu-renorm-layer name=tdnn_5 dim=512 input=Append(-3,0,3)
  relu-renorm-layer name=tdnn_6 dim=512 input=Append(-6,-3,0)

  relu-renorm-layer name=prefinal-chain dim=512 target-rms=0.5
  output-layer name=output include-log-softmax=false dim=$num_targets max-change=1.5
  
  relu-renorm-layer name=prefinal-xent input=tdnn_6 dim=512 target-rms=0.5
