...
dir=exp/chain${nnet3_affix}/upper_bound/tdnn_${affix}_clean/${argu_desc}
train_data_dir=my_data/clean_noisy/${train_set}_sp_hires
...
if [ $stage -le 12 ]; then
  ...
  dim-range-component name=origin input=input dim=40 dim-offset=40
  ...
  no-op-component name=input2 input=Append(context-clean, delta, Scale(1.0, ReplaceIndex(ivector, t, 0)))
    ...
    (
      ...
      for lmtype in tgpr_5k; do
        steps/nnet3/decode.sh \
          ...
          $tree_dir/graph_${lmtype} my_data/clean_noisy/${data}_hires ${dir}/decode_${lmtype}_${data_affix} || exit 1
      done
