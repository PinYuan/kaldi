...
argu_desc="e${num_of_epoch}_f${frame_weight}_il${initial_effective_lrate}_fl${final_effective_lrate}_prefinal-xent.batchnorm2"
...
if $use_ihm_ali; then
  ...
  dir=exp/chain${nnet3_affix}/ppg/tdnn_${affix}_sp_ihmali/${argu_desc}
  ...
  dir=exp/chain${nnet3_affix}/ppg/tdnn_${affix}_sp/${argu_desc}
  lores_train_data_dir=data/${train_set}_sp
  tree_dir=exp/chain${nnet3_affix}/tree_a_sp
fi

target_scp=exp/nnet3/ppg/train_si84_clean_sp_hires/prefinal-xent.batchnorm2/target.scp
...
if [ $stage -le 12 ]; then
  ...
  output name=output_ae input=prefinal-xent objective-type=quadratic
EOF
...
if [ $stage -le 13 ]; then
  ...
  steps/nnet3/chain/train_dcae.py --stage=$train_stage \
    --cmd="$train_cmd" \
