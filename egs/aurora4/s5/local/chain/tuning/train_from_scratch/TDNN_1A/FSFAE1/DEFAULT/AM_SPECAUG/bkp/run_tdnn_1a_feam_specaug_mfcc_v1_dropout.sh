...
dir=exp/chain${nnet3_affix}/train_from_scratch/TDNN_1A/FSFAE1/DEFAULT/AM_SPECAUG/tdnn_1a_feam_specaug_mfcc_v1_dropout/${argu_desc}
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=tdnn1 $tdnn_opts dim=1024 input=Append(-2,-1,0,1,2)
  relu-renorm-layer name=tdnn2 $tdnn_opts dim=1024 input=Append(-1,2)
  relu-renorm-layer name=tdnn3 $tdnn_opts dim=1024 input=Append(-3,3)
  relu-renorm-layer name=tdnn4 $tdnn_opts dim=1024 input=Append(-7,2)
  relu-renorm-layer name=tdnn5 $tdnn_opts dim=1024
