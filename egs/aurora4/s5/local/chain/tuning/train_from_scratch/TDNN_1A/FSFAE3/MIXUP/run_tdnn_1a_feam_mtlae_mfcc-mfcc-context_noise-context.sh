...
remove_egs=false
num_of_epoch=30
frame_weight_dae=0.2
frame_weight_dspae=0.2
...
dir=exp/chain${nnet3_affix}/train_from_scratch/TDNN_1A/FSFAE3/MIXUP/tdnn_1a_feam_mtlae_mfcc-mfcc-context_noise-context/${argu_desc}
...
if [ $stage -le 12 ]; then
  ...
  no-op-component name=input2 input=Append(prefinal-dae@-1,prefinal-dae@0,prefinal-dae@1, delta, prefinal-dspae@-1, prefinal-dspae@0, prefinal-dspae@1, Scale(1.0, ReplaceIndex(ivector, t, 0)))
...
if [ $stage -le 13 ]; then
  ...
  steps/chain/train_mtlae_mixup.py --stage=$train_stage \
    --cmd="$train_cmd" \
