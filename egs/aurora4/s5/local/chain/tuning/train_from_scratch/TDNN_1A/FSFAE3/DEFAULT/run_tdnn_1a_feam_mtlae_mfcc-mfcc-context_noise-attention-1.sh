...
dir=exp/chain${nnet3_affix}/train_from_scratch/TDNN_1A/FSFAE3/DEFAULT/tdnn_1a_feam_mtlae_mfcc-mfcc-context_noise-attention-1/${argu_desc}
...
if [ $stage -le 12 ]; then
  ...
  attention-relu-renorm-layer name=attention-dspae input=prefinal-dspae time-stride=3 num-heads=1 value-dim=40 key-dim=20 num-left-inputs=5 num-right-inputs=2

  # AM
  idct-layer name=idct input=input dim=40 cepstral-lifter=22 affine-transform-file=$dir/configs/idct.mat
  delta-layer name=delta input=idct
  no-op-component name=input2 input=Append(prefinal-dae@-1,prefinal-dae@0,prefinal-dae@1, attention-dspae, delta, Scale(1.0, ReplaceIndex(ivector, t, 0)))
