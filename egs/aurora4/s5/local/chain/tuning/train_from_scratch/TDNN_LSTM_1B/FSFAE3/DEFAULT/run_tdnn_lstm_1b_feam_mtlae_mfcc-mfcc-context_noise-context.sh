...
dir=exp/chain${nnet3_affix}/train_from_scratch/TDNN_LSTM_1B/FSFAE3/DEFAULT/feam_mtlae_mfcc-mfcc-context_noise-context/${argu_desc}
...
if [ $stage -le 15 ]; then
  ...
  no-op-component name=context-dspae input=Append(prefinal-dspae@-2, prefinal-dspae@-1, prefinal-dspae@0, prefinal-dspae@1, prefinal-dspae@2)
  affine-layer name=lda input=Append(context-dae, context-dspae, context-acous, ReplaceIndex(ivector, t, 0))
  
  # the first splicing is moved before the lda layer, so no splicing here
  relu-batchnorm-layer name=tdnn1-am $tdnn_opts dim=448
  relu-batchnorm-layer name=tdnn2-am $tdnn_opts dim=448 input=Append(-1,0,1)
  relu-batchnorm-layer name=tdnn3-am $tdnn_opts dim=448 input=Append(-3,0,3)
  relu-batchnorm-layer name=tdnn4-am $tdnn_opts dim=448 input=Append(-3,0,3)
