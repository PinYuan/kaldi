...
argu_desc="e${num_of_epoch}_f${frame_weight}_il${initial_effective_lrate}_fl${final_effective_lrate}_pl1"
...
dir=exp/chain${nnet3_affix}/pretrain/TDNN_1G/FSFAE3/tdnn_${affix}_feam_baseline/${argu_desc}
...
if [ $stage -le 15 ]; then
  ...
  relu-renorm-layer name=tdnn0 dim=1024
  # output-layer name=output_ae include-log-softmax=false learning-rate-factor=1 objective-type=quadratic input=tdnn5 dim=40

  # please note that it is important to have input layer with the name=input
  # as the layer immediately preceding the fixed-affine-layer to enable
  # the use of short notation for the descriptor
  fixed-affine-layer name=lda input=Append(input@-1,input@0,input@1,ReplaceIndex(ivector, t, 0)) affine-transform-file=$dir/configs/lda.mat

  # the first splicing is moved before the lda layer, so no splicing here
  relu-batchnorm-dropout-layer name=tdnn6 input=Append(lda,tdnn0) $tdnn_opts dim=1024
...
if [ $stage -le 16 ]; then
  # Modify pretrained model output-layer name
  # pretrained_mdl="/mnt/HDD2/user_pinyuanc/kaldi/egs/aurora4/s5/exp/nnet3/denoising_autoencoder/baseline/e10_il0.001_fl0.0001/final.raw"
  # pretrained_mdl="/mnt/HDD2/user_pinyuanc/mod-kaldi/kaldi-slam/egs/aurora4/s5/exp/chain/seperate/tdnn_1a_feam_baseline/e30_f1_il0.01_fl0.001/ext_dae.raw"
  
  mkdir -p $dir/configs

  pretrained_mdl="/mnt/HDD2/user_pinyuanc/mod-kaldi/kaldi-slam/egs/aurora4/s5/exp/chain/seperate/tdnn_1a_feam_baseline/e100_f1_il0.01_fl0.001/ext_dae.raw"
  nnet3-copy --edits="rename-node old-name=output new-name=output_ae;" \
            $pretrained_mdl $dir/dae.raw
 
  # Combined with chain model's layers
  cp "exp/chain/pretrain/FSFAE3/tdnn_1g_feam_baseline/e10_pl0/configs/add.config" "$dir/configs/add.config"
  nnet3_add_config="$dir/configs/add.config"
  nnet3-init $dir/dae.raw $nnet3_add_config $dir/am.raw

  # Modify pretrained model's layers learning-rate
  primary_lr_factor=1;
  nnet3-copy --edits="set-learning-rate name=tdnn1* learning-rate=$primary_lr_factor;
                      set-learning-rate name=tdnn2* learning-rate=$primary_lr_factor;
                      set-learning-rate name=tdnn3* learning-rate=$primary_lr_factor;
                      set-learning-rate name=tdnn4* learning-rate=$primary_lr_factor;
                      set-learning-rate name=tdnn5* learning-rate=$primary_lr_factor;
                      set-learning-rate name=output_ae* learning-rate=$primary_lr_factor;
                      set-learning-rate-factor name=tdnn1* learning-rate-factor=$primary_lr_factor;
                      set-learning-rate-factor name=tdnn2* learning-rate-factor=$primary_lr_factor;
                      set-learning-rate-factor name=tdnn3* learning-rate-factor=$primary_lr_factor;
                      set-learning-rate-factor name=tdnn4* learning-rate-factor=$primary_lr_factor;
                      set-learning-rate-factor name=tdnn5* learning-rate-factor=$primary_lr_factor;
                      set-learning-rate-factor name=output_ae* learning-rate-factor=$primary_lr_factor;
                     " \
        $dir/am.raw $dir/input.raw 
  
  # Initialize AM model # seems duplicate
  copy-transition-model $tree_dir/final.mdl $dir/0.trans_mdl
  nnet3-am-init $dir/0.trans_mdl $dir/input.raw $dir/0.mdl
fi
...
if [ $stage -le 17 ]; then
  ...
  steps/chain/train_dcae.py \
    --stage=$train_stage \
    --cmd="$decode_cmd" \
    --trainer.input-model $dir/0.mdl \
