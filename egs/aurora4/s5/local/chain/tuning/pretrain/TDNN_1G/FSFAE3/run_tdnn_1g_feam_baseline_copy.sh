...
argu_desc="e${num_of_epoch}_f${frame_weight}_il${initial_effective_lrate}_fl${final_effective_lrate}_pl${primary_lr_factor}_fsf1"
...
tree_dir=exp/chain${nnet3_affix}/tree_a_sp # _fsf1_asf1
...
if [ $stage -le 14 ]; then
  ...
  steps/nnet3/chain/build_tree.sh \
    ...
    --context-opts "--context-width=2 --centraalignment-subsampling-factorl-position=1" \
...
if [ $stage -le 16 ]; then
  ...
  nnet3-copy --edits="rename-node old-name=output.affine new-name=output_ae.affine;
                      rename-node old-name=output new-name=output_ae;" \
            $pretrained_mdl $dir/dae.raw
  sed -i 's/output.affine/output_ae.affine/g' $dir/dae.raw
  
  # Combined with chain model's layers
  nnet3_add_config="$dir/configs/add.config"
  # nnet3_add_config="$dir/configs/add_sep.config"
