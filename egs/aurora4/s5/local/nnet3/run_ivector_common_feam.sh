...
stage=0
nj=30
train_set=train_si84_multi   # you might set this to e.g. train.
target_set=train_si84_clean   # you might set this to e.g. train.
...
for f in data/${train_set}/feats.scp data/${target_set}/feats.scp ${gmm_dir}/final.mdl; do
  if [ ! -f $f ]; then
...
if [ $stage -le 5 ] && [ -f data/${train_set}_sp_hires/feats.scp ] && \
   [ -f data/${target_set}_sp_hires/feats.scp ]; then
...
if [ $stage -le 5 ]; then
  ...
  for datadir in ${train_set}_sp ${target_set}_sp ${test_sets}; do
