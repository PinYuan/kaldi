...
cat $dir/scoring/text.filt | awk '{{for (i = 2; i <= NF; i++) printf(" %s", $i);} printf("\n"); }' | perl -CIOED -p -e 's/\p{Block=CJK_Unified_Ideographs}/ $& /g' | sed 's/  / /g' > $dir/scoring/utt_tra
paste -d' ' $dir/scoring/utt_id $dir/scoring/utt_tra | sed 's/  / /g' > $dir/scoring/char.filt

for lmwt in `seq $min_lmwt $max_lmwt`; do
  cat $dir/scoring/$lmwt.txt | awk '{ print $1}' > $dir/scoring/utt_id
  cat $dir/scoring/$lmwt.txt | awk '{{for (i = 2; i <= NF; i++) printf(" %s", $i);} printf("\n"); }' | perl -CIOED -p -e 's/\p{Block=CJK_Unified_Ideographs}/ $& /g' | sed 's/  / /g'  > $dir/scoring/utt_tra
  paste -d' ' $dir/scoring/utt_id $dir/scoring/utt_tra | sed 's/  / /g' > $dir/scoring/${lmwt}.char
done
