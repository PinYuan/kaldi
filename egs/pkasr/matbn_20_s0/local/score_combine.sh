...
cmd=run.pl
beam=10 # prune the lattices prior to MBR decoding, for speed.
...
for lmwt in $(seq $min_lmwt $max_lmwt); do
    filter_text <$dir/scoring/penalty_$word_ins_penalty/$lmwt.txt >$dir/scoring/penalty_$word_ins_penalty/$lmwt.txt.filt
    mv $dir/scoring/penalty_$word_ins_penalty/$lmwt.txt.filt $dir/scoring/penalty_$word_ins_penalty/$lmwt.txt
done
...
if [ $stage -le 2 ] ; then
  ...
  files=($dir/scoring/test_filt.txt)
