...
import libs.nnet3.train.chain_objf.acoustic_model_mixup as chain_lib
...
def get_args():
    ...
    parser.add_argument("--egs.frame-weight-dae", type=float, dest='frame_weight_dae',
                        default=1.0,
                        help="""For mtlae training with multi-condition data. Set 
    ...
    parser.add_argument("--target_scp_dae", type=str, required=True,
                        help="Directory with target_scp_dar for MTLAE")
    parser.add_argument("--target_scp_dspae", type=str, required=True,
                        help="Directory with target_scp_dspae for MTLAE")
...
def train(args, run_opts):
    ...
    if ((args.stage <= -3) and args.egs_dir is None):
        ...
        common_lib.execute_command(
            """steps/nnet3/chain/get_egs_mtlae.sh {egs_opts} \
                --cmd "{command}" \
                --cmvn-opts "{cmvn_opts}" \
