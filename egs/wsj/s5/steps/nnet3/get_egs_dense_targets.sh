...
num_archives=$[$num_frames/($frames_per_eg_principal*$samples_per_iter*8192)+1]
...
egs_per_archive=$[$num_frames/($frames_per_eg*$num_archives*8192)]
...
if [ $stage -le 5 ]; then
  ...
  if [ $archives_multiple == 1 ]; then # normal case.
    $cmd --max-jobs-run $nj JOB=1:$num_archives_intermediate $dir/log/shuffle.JOB.log \
      nnet3-shuffle-egs --srand=JOB "ark:cat $egs_list|" ark,scp:$dir/egs.JOB.ark,$dir/egs.JOB.scp  || exit 1;
  else
...
cat $dir/egs.*.scp > $dir/egs.scp
# cat $dir/targets.*.scp > $dir/egs.scp
#for f in $dir/targets.*.scp; do rm $f; done
