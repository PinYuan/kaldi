...
affix=1g_avector   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if [ $stage -le 15 ]; then
  ...
  input dim=100 name=avector
  input dim=40 name=input

  # please note that it is important to have input layer with the name=input
  # as the layer immediately preceding the fixed-affine-layer to enable
  # the use of short notation for the descriptor
  fixed-affine-layer name=lda input=Append(-1,0,1,ReplaceIndex(ivector, t, 0),ReplaceIndex(avector, t, 0)) affine-transform-file=$dir/configs/lda.mat
...
if [ $stage -le 16 ]; then
  ...
  steps/nnet3/chain/train.py --stage=$train_stage \
    ...
    --feat.avector-dir=$train_ivector_dir \
