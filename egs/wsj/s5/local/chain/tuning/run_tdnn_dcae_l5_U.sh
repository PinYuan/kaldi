...
affix=_l5_U   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if [ $stage -le 15 ]; then
  ...
  relu-batchnorm-layer name=tdnn4 input=Sum(0, Scale($skip_connection_weight, tdnnf2)) dim=1024
  relu-batchnorm-layer name=tdnn5 input=Sum(0, Scale($skip_connection_weight, tdnn1)) dim=1024
