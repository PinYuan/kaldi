...
affix=_l6_U_neg   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if [ $stage -le 15 ]; then
  ...
  relu-batchnorm-layer name=tdnn5 input=Append(0, Sum(Scale($skip_connection_weight, tdnnf2), Scale(-1, 0))) dim=1024
  relu-batchnorm-layer name=tdnn6 input=Append(0, Sum(Scale($skip_connection_weight, tdnn1), Scale(-1, 0))) dim=1024
  output-layer name=output_ae include-log-softmax=false learning-rate-factor=$weight max-change=1.0 objective-type=quadratic input=tdnn6 dim=40
