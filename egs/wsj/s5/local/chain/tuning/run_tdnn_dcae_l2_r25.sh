...
affix=_l2_r25   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if [ $stage -le 15 ]; then
  ...
  output-layer name=output_ae include-log-softmax=false learning-rate-factor=$weight max-change=1.0 objective-type=quadratic input=tdnn2 dim=40
