...
affix=_l3_r25_U_neg   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
dir=exp/chain${nnet3_affix}/tdnn${affix}_e${num_of_epoch}_sp/${weight}_${skip_connection_weight}
train_data_dir=data/${train_set}_sp_hires
train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
lores_train_data_dir=data/${train_set}_sp

target_scp=data/${train_set}_sp_hires/feats.scp
...
if [ $stage -le 15 ]; then
  ...
  tdnnf-layer name=tdnnf14 l2-regularize=0.01 dropout-proportion=0.0 bypass-scale=0 input=tdnnf12 dim=256 bottleneck-dim=128 time-stride=3
  relu-batchnorm-layer name=tdnn2 input=Append(0, Sum(Scale($skip_connection_weight, tdnnf2), Scale(-1, tdnnf13))) dim=1024
  relu-batchnorm-layer name=tdnn3 input=Append(0, Sum(Scale($skip_connection_weight, tdnn1), Scale(-1, 0))) dim=1024
  output-layer name=output_ae include-log-softmax=false learning-rate-factor=$weight max-change=1.0 objective-type=quadratic input=tdnn3 dim=40
