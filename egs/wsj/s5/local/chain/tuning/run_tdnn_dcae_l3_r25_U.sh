...
affix=_l3_r25_U   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if [ $stage -le 15 ]; then
  ...
  relu-batchnorm-layer name=tdnn2 input=Append(0, Sum(tdnnf13, Scale($skip_connection_weight, tdnnf2))) dim=1024
  relu-batchnorm-layer name=tdnn3 input=Sum(0, Scale($skip_connection_weight, tdnn1)) dim=1024
