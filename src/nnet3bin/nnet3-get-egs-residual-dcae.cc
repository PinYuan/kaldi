...
static bool ProcessFile(const GeneralMatrix &feats,
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    ...
    Posterior residuals(num_frames_subsampled);

    // TODO: it may be that using these weights is not actually helpful
    // (with chain training, it was not), and that setting them all to 1 is
    // better. We could add a boolean option to this program to control
    // that; but I don't want to add such an option if experiments show that
    // it is not helpful.
    for (int32 i = 0; i < num_frames_subsampled; i++) {
      ...
      //make label for residual layer of dcae
      while(count<20){
        temp = rand() % num_pdfs;
        int32 found=0;
        for (std::vector<std::pair<int32, BaseFloat> >::iterator iter = residuals[i].begin(); iter != residuals[i].end(); ++iter) {
          if (iter->first == temp) {
              found = 1;
              break;
          }
        ...
        if (temp != ali && found == 0) {
          residuals[i].push_back(std::make_pair(temp, 0.05));
          count++;
        }
      }
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "general)\n"
        "\n"
        "Usage:  nnet3-get-egs-residual-dcae [options] <features-rspecifier> "
        "<pdf-post-rspecifier> <egs-out>\n"
        "\n"
        "An example [where $feats expands to the actual features]:\n"
        "nnet3-get-egs-residual-dcae --num-pdfs=2658 --num-targets=26 --left-context=12 "
        "--right-context=9 "
        "--num-frames=8 \"$feats\"\\\n"
